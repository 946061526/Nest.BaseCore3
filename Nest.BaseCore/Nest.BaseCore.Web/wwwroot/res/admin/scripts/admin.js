...
layui.define(['jquery', 'form', 'layer', 'element'], function (exports) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.Code == 200) {
                    ...
                    layer.msg('请求错误：' + res.Message + '，' + res.Code);
                }
            }
     ...
     * 图片缩放和放大
     */
    $('body').on('click', '.soway-active-image', function () {
        $(this).toggleClass('soway-active-image-max');
    });

    /**
     * 表格列表图片弹出框显示
     */
    $('body').on('click', '.soway-table-list-image', function () {
        var url = $(this).attr('src');
        layer.open({
            type: 1,
            title: false,
            closeBtn: 0,
            skin: 'layui-layer-nobg soway-table-list-image-layer', //没有背景色
            shadeClose: true,
            content: '<img src="' + url + '">'
        });
    });
	...
	*/
    ...
    window.WeAdminShow = function (title, url, w, h) {
        ...
        if (url == null || url == '') {
            url = "/home/Error?msg=程序出错，请联系系统管理员";
        };
        if (w == null || w == '') {
            w = ($(window).width() * 0.9);
        };
